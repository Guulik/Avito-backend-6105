# Дисклеймер
Были некоторые неточности и не очевидные моменты в бизнес-логике, поэтому где-то мне пришлось брать инициативу и принимать решение самостоятельно. Это могло отразиться на баллах за автоматические тесты. [Ниже](README.md#логика) подробно

PS: пришлось экстренно пересобирать проект, поэтому в этот раз без тестовых данных в базе( 
## Как запустить
### Если у вас НЕТ базы
...
Сервис будет доступен по адресу localhost:8080, база будет лежать рядом, но только там будет пусто. Хз как её заполнить быстро в данных условиях)
### Если у вас есть база
Тогда следует запустить контейнер с приложением изолированно. 

Для этого у меня есть команда 

`make run-isolated`

По дефолту опрелеены переменные окружения [всё те же, что были в gitlab-ci.yml](./Dockerfile)

Если надо переопределить переменные окружения, то используйте
